...
export class UserAccountManagementService {
  ...
  searchByKeyWord(search: any, object: any, groups: any, status: any) {
    return this.http
      .post<any>(`${environment.apiUrl}/user/list`, { search, object, groups, status })
      .pipe(delay(1000),
        map(res => {
          return res;
        }));
  ...
  searchByInput(permission_id: any, groups: any, status: any) {
    return this.http.post(`${environment.apiUrl}/user/list`, { permission_id, groups, status })
      .pipe(map(res => {
        return res;
      }));
